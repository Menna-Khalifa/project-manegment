            ...
            <div class="card bg-primary-gradient profile-header-card">
                <div class="card-body">
                    <div class="row align-items-center">
                        <div class="col-lg-3 text-center">
                            <div class="profile-avatar">
                                @if ($user->getFirstMediaUrl('avatars') != null)
                                    <img src="{{ $user->getFirstMediaUrl('avatars', 'avatar') }}" alt="{{ $user->name }}"
                                        class="profile-image">
                                @else
                        ...
                        <div class="col-lg-6">
                            <h3 class="text-white mb-2">{{ $user->name ?? __('general.not_found') }}</h3>
                            <p class="text-white-50 mb-3">{{ $user->email ?? __('general.not_found') }}</p>
                            <div class="d-flex flex-wrap gap-2">
                                <span class="badge badge-light badge-lg">
                                    <i class="fas fa-user-tag me-2"></i>
                                    {{ $user->getRoleNames()->first() ?? __('general.not_found') }}
                                </span>
                                <span class="badge badge-light badge-lg">
                                    <i class="fas fa-users me-2"></i>
                                    {{ $user->group->name ?? __('general.not_found') }}
                                </span>
                                <span class="badge badge-{{ $user->status == 'active' ? 'success' : 'danger' }} badge-lg">
                                    <i
                                        class="fas fa-{{ $user->status == 'active' ? 'check-circle' : 'times-circle' }} me-2"></i>
                                    {{ $user->status == 'active' ? __('admins.enabled') : __('admins.not_enabled') }}
                                </span>
                            </div>
                        </div>
                            ...
                            <div class="profile-stats">
                                <div class="stat-item">
                                    <h4 class="text-white mb-0">
                                        {{ $user->projectTeams ? $user->projectTeams->where('project.status', 'active')->count() : 0 }}
                                    </h4>
                                    <small class="text-white-50">Active Projects</small>
                                </div>
                                <div class="stat-item mt-2">
                                    <h4 class="text-white mb-0">
                                        {{ $user->projectTeams ? $user->projectTeams->count() : 0 }}</h4>
                    ...
                    <div class="info-list">
                        <div class="info-item">
                            ...
                            <div class="info-content">
                                <label>Email</label>
                                <span>{{ $user->email ?? __('general.not_found') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="info-item">
                            ...
                            <div class="info-content">
                                <label>Phone</label>
                                <span>{{ $user->phone ?? __('general.not_found') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="info-item">
                            ...
                            <div class="info-content">
                                ...
                                <span>{{ $user->created_at ? $user->created_at->format('M d, Y') : __('general.not_found') }}</span>
                            </div>
                        </div>
                ...
                <div class="card-body">
                    <div class="stats-grid">
                        <div class="stat-card bg-success-light">
                            ...
                            <div class="stat-info">
                                <h4>{{ $user->projectTeams->where('project.status', 'active')->count() }}</h4>
                        ...
                        <div class="stat-card bg-info-light">
                            ...
                            <div class="stat-info">
                                <h4>{{ $user->projectTeams->where('project.status', 'delivered')->count() }}</h4>
                                <span>Completed</span>
                            </div>
                        </div>
                        ...
                        <div class="stat-card bg-warning-light">
                            ...
                            <div class="stat-info">
                                @php
                                    $activeProjects = $user->projectTeams->where('project.status', 'active');
                                    $totalProgress = 0;
                                    $count = 0;
        ...
        <div class="col-lg-8">
            ...
            <div class="card modern-card">
                <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <span
                        class="badge badge-primary">{{ $user->projectTeams->where('project.status', '!=', 'completed')->count() }}
                        Projects</span>
                </div>
                <div class="card-body p-0">
                    @forelse($user->projectTeams->where('project.status', '!=','completed') as $projectTeam)
                                                    ...
                                                    <div class="row align-items-center">
                                                        <div class="col-3 text-center">
                                                            @if ($user->getFirstMediaUrl('avatars'))
                                                                <img src="{{ $user->getFirstMediaUrl('avatars', 'avatar') }}"
                                                                    alt="{{ $user->name }}" class="transfer-avatar">
                                                            @else
                                                        ...
                                                        <div class="col-9">
                                                            ...
                                                            <div class="mt-2">
                                                                <span
                                                                    class="badge badge-info">{{ $user->getRoleNames()->first() }}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
            ...
            @if ($user->projectTeams->where('project.status', 'completed')->count() > 0)
                <div class="card modern-card mt-4">
                    <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                        <h5 class="mb-0"><i class="fas fa-check-circle me-2"></i>Completed Projects</h5>
                        <span
                            class="badge badge-success">{{ $user->projectTeams->where('project.status', 'completed')->count() }}
                            Projects</span>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover modern-table">
                                ...
                                <tbody>
                                    @foreach ($user->projectTeams->where('project.status', 'completed') as $projectTeam)
                                        @php $project = $projectTeam->project; @endphp
                                        <tr>
                                            <td>
                                            ...
                                            <td>{{ $project->type }}</td>
                                            <td>{{ $project->start_date->diffInDays($project->end_date) }}
                                                days</td>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('update_profile_photo', $user->id) }}" method="POST"
        ...
        document.addEventListener('DOMContentLoaded', function() {
                ...
                select.addEventListener('change', function() {
                    ...
                    if (this.value) {
                        ...
                        confirmButton.innerHTML =
                            '<i class="fas fa-exchange-alt me-1"></i>Transfer to ' + selectedOption
                            .text.split('(')[0].trim();
            ...
            document.querySelectorAll('form[action*="transfer"]').forEach(function(form) {
                form.addEventListener('submit', function(e) {
                    const selectedProject = this.querySelector('.select-project').selectedOptions[
                    0];
                    if (selectedProject) {
                        const projectName = selectedProject.text.split('(')[0].trim();
                        if (!confirm(
                                `Are you sure you want to transfer this team member to "${projectName}"?`
                                )) {
                            e.preventDefault();
                        }
                    }
