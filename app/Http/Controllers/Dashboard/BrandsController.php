...
class BrandsController extends Controller
{
    ...
    {
        $this->middleware('check.permission:brands_list', ['only' => ['index']]);
        $this->middleware('check.permission:add_brand', ['only' => ['create', 'store']]);
        $this->middleware('check.permission:edit_brand', ['only' => ['edit', 'update']]);
        $this->middleware('check.permission:delete_brand', ['only' => ['delete']]);
    }
    ...
    public function ajaxStore(BrandRequest $request)
    {
        try {
            ...
            Log::error('Error in BrandsController@ajaxStore: ' . $e->getMessage());
    ...
    public function update(BrandRequest $request, Brand $brand)
    {
        try {
            ...
            Log::error('Error in BrandsController@update: ' . $e->getMessage());
    ...
    public function destroy(Brand $brand)
    {
        try {
            ...
            if ($brand->stores()->count() > 0) {
                notify(__('brands.delete_brand_error_stores_exist'), 'error');
                return redirect()->back()->withErrors(['error' => __('brands.delete_brand_error_stores_exist')]);
            }
